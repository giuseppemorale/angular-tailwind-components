...
Use **`provideTailwindConfig`** to override library **injection tokens** (`ICON_SIZE`, `DATETIME_LANGUAGE`, `COMPONENTS_SIZE`, `BUTTON_KIND`, `PAGINATION_SUMMARY`, `PASSWORD_LABELS`, `EDITOR_LABELS`, `TITLE_SCALE`). Pass a **factory** so you can use `inject()` (e.g. Transloco). Values resolve on first token injection (after your app initializers).

For runtime semantic **`COLORS`** on `:root`, add **`provideTailwindThemeColors`** separately.
...
This library is licensed under the **Angular Tailwind Components License 1.0 (ATC-1.0)**. See the [LICENSE](https://github.com/giuseppemorale/angular-tailwind-components/blob/master/LICENSE) file for the full text.
...
**Third-party assets** bundled with this package keep their original licenses and are not covered by ATC-1.0. In particular, the bundled **[Heroicons](https://heroicons.com/)** outline SVG icons are © [Tailwind Labs](https://tailwindcss.com/), licensed under the [MIT License](https://github.com/tailwindlabs/heroicons/blob/master/LICENSE).
