...
Use **`provideTailwindComponents`** from `angular-tailwind-components` to register defaults for **`TAILWIND_ICON_SIZE`**, **`TAILWIND_DATETIME_LANGUAGE`**, **`TAILWIND_COMPONENTS_SIZE`**, **`TAILWIND_PAGINATION_SUMMARY`**, and optionally **`TAILWIND_MODAL_DATA`**. It returns **`EnvironmentProviders`**: add it directly to `bootstrapApplication` / `ApplicationConfig.providers` (no spread). See the package README for the property → token table and a full example.

Use **`defineTheme`** to remap semantic colors (`primary`, `neutral`, `success`, `warning`, `danger` / `error`, `info`) at runtime via CSS variables on `document.documentElement`. It returns **`EnvironmentProviders`** the same way. For string palette values, use names from the [Tailwind color reference](https://tailwindcss.com/docs/colors). Full options, `TailwindThemeSeverityColor`, and examples are in the [package README](https://github.com/giuseppemorale/angular-tailwind-components/blob/main/projects/angular-tailwind-components/README.md#theme-colors-definetheme).
