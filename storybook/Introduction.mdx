...
Use **`defineTheme`** from `angular-tailwind-components` for app-wide **injection token** defaults. It returns **`EnvironmentProviders`**: add one entry to `bootstrapApplication` / `ApplicationConfig.providers` (no spread).
...
Set accents per component with **`color`** (`blue`, `indigo`, `emerald`, …). See the [Tailwind color reference](https://tailwindcss.com/docs/colors) and the [package README](https://github.com/giuseppemorale/angular-tailwind-components/blob/main/projects/angular-tailwind-components/README.md#application-configuration-definetheme).
