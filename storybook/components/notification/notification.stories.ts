...
export const Notification: StoryObj<TailwindNotification> = {
  render: args => ({
    props: args,
    template: `
      <tailwind-notification [color]="color" [title]="title" [dismissible]="dismissible">
        This is a notification message with relevant details.
      </tailwind-notification>`
  }),
  args: { color: 'sky', title: 'Information', dismissible: true }
};
  ...
  render: () => ({
    template: `
      <div class="flex flex-col gap-3 max-w-lg">
          <tailwind-notification color="amber" title="Storage almost full" [showActions]="true">
          You are using 4.8 GB of your 5 GB storage limit.
          <div tailwind-notification-actions>
            <div class="flex gap-2">
              <tailwind-button size="sm" color="amber" kind="outlined">Manage storage</tailwind-button>
              <tailwind-button size="sm" color="amber" kind="text">Dismiss</tailwind-button>
            </div>
          </div>
        </tailwind-notification>
        <tailwind-notification color="sky" title="New update available" [showActions]="true">
          Version 2.0 includes performance improvements and new features.
          <div tailwind-notification-actions>
            <div class="flex gap-2">
              <tailwind-button size="sm">Update now</tailwind-button>
              <tailwind-button size="sm" color="neutral" kind="text">Later</tailwind-button>
            </div>
          </div>
        </tailwind-notification>
      </div>`
